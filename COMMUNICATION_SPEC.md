...
### RT の損害〜復旧の一連（§4.3.4 / §4.3.5 / §4.2.2h / §4.2.1j）— ✅ 完了
- `checkRTCombatDamage(unitId)`：**最後の（唯一の）ステップが Casualty になった**駒の
  RT ごとに **R#1/2** を振る。破壊なら `removedRTs`（Removed from Play）へ、
  残れば `droppedRTMap` に入れて**そのカードに落とす**。どちらでもその駒からは失われる
- 呼び出し口は `hit.js` の Casualty 経路3か所
  （namedFireTeam の C ／ steps===2 の C ／ combo の l1==='C'）
- `pickUpRT()`：§4.2.2h。1コマンド消費、拾った駒は **Exposed**。
  発令者は「その駒に命令できる HQ/Staff」を自動で探す（通信・コマンド上限も見る）
- `switchRTNetwork()`：§4.2.1j。**同じ機種**で、**Removed from Play になった網**にだけ
  載せ替えられる（例: SCR300 BN TAC が破壊された → SCR300 Mtr FD を BN TAC へ）。
  載せ替えると `removedRTs` からその枠が消える
- カード右クリックに「📻 落ちている無線・電話」欄（拾う駒を選んで1コマンドで回収）
- カード上に RT カウンター（`images/Net - {網} - {機種}.png`）を描画
- 状態は play 層に保存

### 未実装（意図的に見送り・優先度順）
1. §4.3.4 移動時の自動敷設（カードを離れるとき1本置く・命令不要）
