...
| 4.2 アクション | 🟡 | §4.2.1 C&C アクションは実装済（Activate a / Runner f-h / 網載せ替え j / 電話線修理 k / 拾う 4.2.2h）。**§4.2.3 Rally も実装済**（rally.js・a〜f と j。VOF無しなら自動成功／あれば「2±発令者の練度」枚引いて `type:'rally'` を探す・§6.5.1）。**移動 §4.2.2・戦闘 §4.2.4 は未**（Rally の g/h/i も未）。**調査済みの実装計画は ACTION_SPEC.md**（Auto/ドローの4類型・着手順 Rally→移動→戦闘・各アクションのコスト/Draw表つき） |
