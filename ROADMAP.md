...
| 3.7 相互戦闘 | 🟡 | 戦闘解決(6.4)は手動ドローで実装済。PC解決(§8.2.4)・接触タイプ判定(§8.3)は実装済。**§3.7.3 Pinned回復も実装済**（cleanup.js・クリーンアップフェーズ到達時にまとめて処理。詳細は3.8参照） |
| 3.8 クリーンアップ | 🟡 | **cleanup.js で実装**：フェーズが「クリーンアップ」に進んだ瞬間に ①§3.7.3 Pinned Recovery（VOFの無いカードのユニットのPinnedを解除）②§3.8 Exposed マーカー除去 ③Concentrated Fire/Grenade/Grenade Miss マーカー除去（持続系VOFのS/A/H等は対象外）を実行。コマンドの起動/取得済みフラグ・AP繰越（`resetImpulseFlags`/`finishImpulse`）は既存実装と連動。**未実装**（新規下位システムが要るため）: Pyrotechnic/Smoke/Illumination除去(§4.4/9.2)・Casualty Evacuation(§5.1.7)・VOFを持たない敵への自動Cease Fire(§8.6敵AI)・防御ミッションの未解決PC除去(§3.2)・Mine/Sniper VOFの処理(§8.7/7.15) |
...
| 節 | 状況 | メモ |
|----|------|------|
| 5.1 移動 | 🟡 | ドラッグ移動可。**§4.2.2 の命令としての移動を実装**（move.js）：1コマンド消費・隣接判定・**Exposed化と §5.1.2 の例外**（塹壕/バンカー/トーチカ間は付かない）・カバーの出入り・§4.2.5 Pinned の移動先制限・§4.3.4 離脱時の電話線自動敷設。**浸透 c/g・カバー捜索 e も右パネル UI 接続済み**（詳細は 4.2 アクションの行を参照）。**§5.1.6E の物資投棄・Jitter/Stagger は未** |
...
| 6.5 Rally・再編 | ✅ | **§6.5.1 Rally 実装済**（rally.js）：VOF無しなら自動成功／あれば「2±発令者の練度」枚引いて `type:'rally'` を探す。Pinned解除・LAT変換(Paralyzed→Litter→FT→AT)・Fire Team面の表裏・Detach Team(g)・Supplement Squad(h)に対応。**§6.5.2/§4.2.3i Reconstitute Squad も実装済み**（reconstitute.js）：分隊がFire/Assault Teamだけ残して盤上から消える瞬間(hit.jsの消滅閾値分岐)を"Removed from Play"としてプールに記録し、2〜4体のUnpinned Team＋同じプールのエントリを消費して分隊を復活させる。カード右クリックメニューにUI実装（PC解決と同型のドローロック付きステップフロー） |
...
| 8.6 敵の挙動(Behavior) | 🟡 | **§8.6.2 Activity Check 実装済み**（enemy-activity.js・data/enemy-activity.js）。3枚の実物チャート画像（Pinned/LAT・Offensive・Defensive Hierarchy）を書き起こし、カードごとにPinned/LAT→Good Order→Leaderの順で1体ずつ判定→R#ドロー→行動決定まで自動化。**自動適用は No Action・Fall Back(8.6.3)・除去+PC設置の3行動のみ**、それ以外（Rally/Grenade/Concentrate/Infiltrate/Reconstitute/PDFシフト）は指示表示のみで既存UIから手動実行。判定条件（同カード/カバー/Out of Ammo/LOS/被弾中）は自動判定だが人間が右パネルで上書き可能。詳細・既知の簡略化とチャート読み取りの確信度は ENEMY_ACTIVITY_SPEC.md 参照。§8.7-8.12（地雷・スナイパー・Leader派生・Spotter・弾薬切れVOF降格・車両）は未 |
...
7. ~~**クリーンアップ処理**（3.8）— Exposed解除・Pinned回復（3.7.3）~~ **完成**（cleanup.js）。
   ...
   **呼び出し元が無い**（§3.7.4 Combat Effects Segment 相当のフェーズ処理が未実装なため。
   友軍/敵の Good Order 占有判定を渡す必要があり、単純にクリーンアップへ足すのは時期尚早）
...
### ★P4：敵の厚み
11. ~~敵の挙動(Activity Check)（8.6）~~ **判定ロジックは完成**（enemy-activity.js。詳細はENEMY_ACTIVITY_SPEC.md）。自動適用3行動以外は手動実行前提
